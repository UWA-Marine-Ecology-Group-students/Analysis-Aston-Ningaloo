...
out.of.range<-filter(length,range>7000)%>% # 10 m = 10000 mm
...
length.species.not.previously.observed<-master%>%
  ...
  filter(!species%in%c("spp", "sp1", "sp10"))%>% # Ignore spp in the report
  glimpse()
...
taxa.maxn.vs.stereo.summary<-length%>%
  ...
  select(sample,family,genus,species,maxn,stereo.maxn,difference,percent.difference)%>%
  arrange(-difference)%>%
  glimpse()
