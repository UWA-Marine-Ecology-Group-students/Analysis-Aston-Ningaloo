...
metadata<-read_csv(file=paste(study,"checked.metadata.csv",sep = "."),na = c("", " "))%>%
  dplyr::mutate(id=paste(sample,sep="."))%>%
  glimpse()
...
complete.length.number<-read_csv(file=paste(study,"checked.length.csv",sep = "."))%>% #na = c("", " "))
  filter(!family=="Unknown")%>%
  dplyr::mutate(id=paste(sample,sep="."))%>%
  dplyr::right_join(metadata ,by = c("id", "sample"))%>% # add in all samples
  dplyr::select(id,sample,family,genus,species,length,number,range)%>%
  tidyr::complete(nesting(id,sample),nesting(family,genus,species)) %>%
