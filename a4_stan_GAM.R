...
effectdata <- expand.grid(cube.Aspect=seq(min(legal.dat$cube.Aspect),max(legal.dat$cube.Aspect),length.out = 20),
  ...
  distinct()%>%
  glimpse()
## calculate unique variance and unique "effect size"
fits.all=list()
mod.dat.all=list()
out.fits.all=list()
mcmc.out=list()
...
mod.list <- list(lme4.site=lme4.site)
...
# Generate stan_Gam
stangam.s <- stan_gamm4(response~s(bathymetry,k=5,bs='cr')+ s(distance.to.ramp, k=5, bs='cr') + s(cube.Aspect,k=5,bs='cr'),
                        random=~(1|site),
                        data=legal.dat, chains=3, cores=3, iter=21000, warmup=20000,
                        family=gaussian())
