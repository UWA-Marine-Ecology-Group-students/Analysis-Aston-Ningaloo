...
dat.var.label<-dat.var.imp%>%
  mutate(label=NA)%>%
  mutate(label=ifelse(predictor=="log.roughness"&resp.var=="Legal","X",ifelse(predictor=="status"&resp.var=="Legal","X",label)))%>%
  mutate(label=ifelse(predictor=="distance.to.ramp"&resp.var=="Sublegal","X",ifelse(predictor=="bathymetry"&resp.var=="Sublegal","X",ifelse(predictor=="cube.aspect"&resp.var=="Sublegal","X",label))))%>%
  glimpse()
...
gg.importance.scores <- ggplot(dat.var.label, aes(x=predictor,y=resp.var,fill=importance))+
  ...
  scale_x_discrete(limits=c("bathymetry","TPI","sqrt.slope","cube.aspect","log.roughness","FlowDir",
                            "distance.to.ramp", "status"),
                   labels=c(
                     "Bathymetry","TPI","Slope (sqrt)","Aspect (cubed)","Roughness (log)","FlowDir",
                     "Distance to Ramp", "Status"
                   ))+
...
gamm.legal<-gam(response~ s(log.roughness, by=status, k=3, bs="cr")+
           s(site,bs="re") +s(TPI,bs="re"), family=tw(),data=dat.legal)
...
predicts.status.by.roughness = testdata%>%data.frame(fits)%>%
  group_by(log.roughness, status)%>% #only change here
  summarise(response=mean(fit),se.fit=mean(se.fit))%>%
  ungroup()
write.csv(predicts.status.by.roughness,"predict.status.by.roughness.csv") #there is some BUG in dplyr - that this fixes
predicts.status.by.roughness<-read.csv("predict.status.by.roughness.csv")%>%
  glimpse()

# Plot Roughness by status from model

ggmod.legal.roughness.status<- ggplot(aes(x=log.roughness,y=response,colour=status), data=dat.legal) +
  ylab("Predicted Abundance of Legal Sized Fish")+
  xlab('Log of Roughness')+
  ...
  geom_line(data=predicts.status.by.roughness,show.legend=FALSE)+
  geom_line(data=predicts.status.by.roughness,aes(y=response - se.fit),linetype="dashed",show.legend=TRUE)+
  geom_line(data=predicts.status.by.roughness,aes(y=response + se.fit),linetype="dashed",show.legend=TRUE)+
  theme_classic()+
  Theme1+
  annotate("text", x = -Inf, y=Inf, label = "(b)",vjust = 1, hjust = -.1,size=5)
ggmod.legal.roughness.status


######## Predict and plot Sublegal bathymetry, sd.relief, distance to ramp ######
dat.sublegal<-dat%>%filter(model=="Sublegal")
gamm.sublegal=gam(response~s(bathymetry,k=3,bs='cr')+s(cube.Aspect,k=3,bs='cr')+ s(distance.to.ramp,k=3,bs='cr')+
                 s(site,bs="re") + s(TPI, bs="re"), family=tw(),data=dat.sublegal)

###### CHECK for spatial autocorrelation in the model
spatial.dat.sublegal <- dat.sublegal
...
#predict bathymetry
mod<-gamm.sublegal
testdata <- expand.grid(cube.Aspect=seq(min(dat$cube.Aspect),max(dat$cube.Aspect),length.out = 20),
...
predict.sublegal.cube.Aspect = testdata%>%data.frame(fits)%>%
  group_by(cube.Aspect)%>% #only change here
  summarise(response=mean(fit),se.fit=mean(se.fit))%>%
  ungroup()
write.csv(predict.sublegal.cube.Aspect,"predict.sublegal.cube.Aspect.csv") #there is some BUG in dplyr - that this fixes
predict.sublegal.cube.Aspect<-read.csv("predict.sublegal.cube.Aspect.csv")%>%
  glimpse()

setwd(m.dir)
#cube.aspect
ggmod.sublegal.cube.Aspect<- ggplot() +
  ylab("Predicted Abundance of Sublegal Sized Fish")+
  xlab('Aspect (cubed)')+
  #   ggtitle(substitute(italic(name)))+
  scale_color_manual(labels = c("Fished", "No-take"),values=c("royalblue2", "slategrey"))+
  #   geom_jitter(width = 0.25,height = 0)+
  geom_point(data=dat.sublegal,aes(x=cube.Aspect,y=response,colour=status),  alpha=0.75, size=2,show.legend=FALSE)+
  geom_line(data=predict.sublegal.cube.Aspect,aes(x=cube.Aspect,y=response),alpha=0.5)+
  geom_line(data=predict.sublegal.cube.Aspect,aes(x=cube.Aspect,y=response - se.fit),linetype="dashed",alpha=0.5)+
  geom_line(data=predict.sublegal.cube.Aspect,aes(x=cube.Aspect,y=response + se.fit),linetype="dashed",alpha=0.5)+
  theme_classic()+
  Theme1+
  annotate("text", x = -Inf, y=Inf, label = "(c)",vjust = 1, hjust = -.1,size=5)
ggmod.sublegal.cube.Aspect
...
predicts.sublegal.ramp = testdata%>%data.frame(fits)%>%
  ...
  summarise(response=mean(fit),se.fit=mean(se.fit))%>%
  ungroup()
write.csv(predicts.sublegal.ramp,"predict.sublegal.ramp.csv") #there is some BUG in dplyr - that this fixes
predicts.sublegal.ramp<-read.csv("predict.sublegal.ramp.csv")%>%
  glimpse()
