...
# Creating the mesh
my.mesh.land <- inla.mesh.2d(Locations, boundary=new_sp, max.edge = c(3500)) #This is the modified one
...
spat.priors <- inla.spde2.pcmatern(
  ...
  prior.sigma=c(0.5, 0.08)) ### 0.5,0.08 for legal, not sure for sub-legal
...
my.stack <- inla.stack(data=list(y=data.legal$target.fish), A=list(A.matrix, 1),
                       ...
                       effects=list(c(mesh.index, list( intercept=1)),#for the spatial effects and intercept
                                    list(bathymetry=data.legal$bathymetry, TPI=data.legal$TPI, Slope=data.legal$Slope, 
                                         Aspect=data.legal$Aspect, FlowDir=data.legal$FlowDir,
                                         mean.relief=data.legal$mean.relief, sd.relief=data.legal$sd.relief, 
                                         reef=data.legal$reef, distance.to.ramp=data.legal$distance.to.ramp, 
